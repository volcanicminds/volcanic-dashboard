...
export function interpretIcon(
  icon: string,
  fields: { alias: string; data: any }[] = []
): string {
  const variableMatch = icon.match(/\${(.*?)}/);
  if (variableMatch) {
    const variableName = variableMatch[1];
    const field = fields.find((field) => field.alias === variableName);
    if (field) {
      return icon.replace(variableMatch[0], field.data);
    }
  }
  return icon;
}

function isTruthy(value: string | number | boolean) {
  ...
  if (_.isNumber(value) || _.isString(value)) {
    return (
      value === "1" || value === 1 || (value as string).toLowerCase() === "true"
    );
  }
...
function isFalsy(value: string | number | boolean) {
  if (_.isBoolean(value)) {
    return !value;
  }
...
export function checkValue(value: string | number | boolean) {
...
export function getBackendError(error: any, defaultErrorCode: string) {
  ...
  return t(defaultErrorCode);
}

export function buildQueryStringWithPrefix(path: string, args: any) {
  let queryString = "";
  ...
  for (const key in args) {
    ...
    // Se il valore è null, undefined, o una stringa vuota, gestiscilo come un parametro senza valore
  ...
  // Ritorna il prefisso con la query string
  return prefix + queryString;
}

// SHA1 Function from: http://locutus.io/php/strings/sha1/
